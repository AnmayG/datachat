...
const CreateChannel = (props: Props) => {
  ...
  const searchSource = useMemo<UserSearchSource>(
    () => {
      const source = new UserSearchSource(client, {debounceMs: 500, pageSize: 6});
      source.activate();
      return source;
    },
    [client],
  ...
  const {users} = useStateStore(searchSource.state, searchSourceStateSelector)
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Enter') {
        ...
        setFocusedUser((focusedUser) => {
          if (focusedUser) addUser(users[focusedUser]);
          return undefined
        });
      }
    }
            ...
            <form>
              <input
                ...
                onChange={(e) => {
                  ...
                  // Debounce the search
                  setTimeout(() => searchSource.search(value), 300);
                }}
